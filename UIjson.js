   ...
   **/
   ...
   $.UIjson.Instance.prototype = {
	...
	defineProp: function (propObj) {
      ...
      if (!$.UIjson.UI_Elements.hasOwnProperty(propObj.assignTo)) { 
	    alert( propObj.assignTo+ ' does not exist in UIjson '); 
		return false; 
	  }
	...
	Obj_initDataholders: function (UIjsonObj) { 
      ...
      $.each( this.Dataholders , function(Ki, propObj) {
	    ...
	    if (UIjsonObj.obj.hasOwnProperty( propObj.prop_name )) { 
		  if (
				(!propObj.path || !UIjsonObj.path) || 
				(propObj.path && UIjsonObj.path && $.UIjson.helpers.checkPath( UIjsonObj.path , propObj.path ))
		  ){
